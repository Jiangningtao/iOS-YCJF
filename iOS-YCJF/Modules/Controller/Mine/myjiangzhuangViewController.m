...
#import "yiguoqiViewController.h"
@interface myjiangzhuangViewController ()<UIScrollViewDelegate>
/***标签栏底部的红色指示器 ***/
...
@implementation myjiangzhuangViewController
...
-(void)setupContentView
{
    ...
    contentView.frame = CGRectMake(0, 100, self.view.frame.size.width, self.view.frame.size.height -54);
...
-(void)setupChildVces
{
    keshiyongViewController *ys = [[keshiyongViewController alloc]init];
    ys.title = @"可使用";
    
    [self addChildViewController:ys];
    
    yishiyongViewController *no = [[yishiyongViewController alloc]init];
    no.title = @"已使用";
    
    [self addChildViewController:no];
    
    yiguoqiViewController *guoqi = [[yiguoqiViewController alloc]init];
    guoqi.title = @"已过期";
    
    [self addChildViewController:guoqi];
...
-(void)leftbtnclicked{
    if ([self.upVC isEqualToString:@"jiangQuan"]) {
